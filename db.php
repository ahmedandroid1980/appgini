	...
	*/
	...
	function db_fetch_array($res){
		switch(DATABASE){
			...
			case 'sqlserver':
			     return array_values(db_fetch_assoc($res));
	...
	function db_field_name($res, $field_offset){
		switch(DATABASE){
			...
			case 'mysqli':
				...
				return $fo->name;
			case 'sqlserver':	
				return odbc_field_name($res,$field_offset);
		}
	}
		...
		switch(DATABASE){
			...
			case 'sqlserver':	
				...
				if($dbname === NULL)
					{	 
					  $dbname = "";
			          $connection_string = "DRIVER={SQL Server};SERVER=$host;";
					}
				else{
					$connection_string = "DRIVER={SQL Server};SERVER=$host;DATABASE=$dbname";
					}
	              $link = odbc_connect($connection_string,$username,$passwd);	

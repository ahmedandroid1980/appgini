<?php
	...
	*/
	...
	if($submit || $test){
		...
		if(!($connection = @db_connect($db_server, $db_username, $db_password,$db_name)))
		{
			$conn = @db_connect($db_server, $db_username, $db_password);
			$qry="CREATE DATABASE $db_name";

			if (! @db_query($qry,$conn))
				{
				$errors[] = @db_error($conn);
				}			
		}
